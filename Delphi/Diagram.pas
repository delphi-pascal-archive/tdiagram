//**********************************************************
// Компонент для отображения графиков для Delphi 3, 5, 7
// Версия - 1.0.8
// Лицензия - GPL v.1
// Автор - TEvg
// **********************************************************
...
uses
  ...
  Windows, SysUtils, Classes, Graphics, Controls, ExtCtrls, Consts;
...
type
  ...
  TCustomDiagram = class(TGraphicControl)
  private
    ...
    function GetColorBackground: TColor; virtual; abstract;
  ...
  private
    ...
    function GetMinX: variant; virtual; abstract;
    procedure SetMinX(Value: variant); virtual; abstract;
    function GetMinY: variant; virtual; abstract;
    procedure SetMinY(Value: variant); virtual; abstract;
    function GetMaxX: variant; virtual; abstract;
    procedure SetMaxX(Value: variant); virtual; abstract;
    function GetMaxY: variant; virtual; abstract;
    procedure SetMaxY(Value: variant); virtual; abstract;
  ...
  public
    { Public declarations }
    property MinX: variant read GetMinX write SetMinX;
    property MaxX: variant read GetMaxX write SetMaxX;
    property MinY: variant read GetMinY write SetMinY;
    property MaxY: variant read GetMaxY write SetMaxY;
  ...
  private
    ...
    procedure SetGridStepX(Value: variant);
    procedure SetGridStepY(Value: variant);
  ...
  public
    ...
    property GridStepX: variant read FGridStepX write SetGridStepX; // шаг сетки по оси абцисс
    property GridStepY: variant read FGridStepY write SetGridStepY; // шаг сетки по оси ординат
    property Visible: boolean read FVisible write SetVisible; // определяет отображать ли координатную сетку
  end;
  ...
  private
    ...
    function GetCurve: TCustomCurve; override;
    procedure SetCurve(lCurve: TCustomCurve); override;
    procedure SetCoordinate(Value: variant);
    procedure SetArrowWidth(Value: integer);
    procedure SetArrowLeng(Value: integer);
    procedure SetVisible(Value: boolean);
    procedure SetVisibleArrow(Value: boolean);
    procedure SetVisibleScale(Value: boolean);
    procedure SetIndentMin(Value: integer);
    procedure SetIndentMax(Value: integer);
    procedure SetScaleStep(Value: variant);
    procedure SetScalePointReference(Value: variant);
    procedure SetAxisType(Value: TAxisType);
    procedure SetDistance(Value: integer);
    procedure SetScaleSize(Value: integer);
    procedure SetScaleEmptyIntervalMin(Value: variant);
    procedure SetScaleEmptyIntervalMax(Value: variant);
    procedure DrawX;
    procedure DrawY;
  public
    { Public declarations }
    ...
    property Coordinate: variant read FCoordinate write SetCoordinate; // координата расположения оси
    property ArrowWidth: integer read FArrowWidth write SetArrowWidth; // ширина стрелки координатной оси, пикселов
    property ArrowLeng: integer read FArrowLeng write SetArrowLeng; // длина стрелки координатной оси, пикселов
    property Visible: boolean read FVisible write SetVisible;       // определяет отображать ли ось
    property VisibleArrow: boolean read FVisibleArrow write SetVisibleArrow; // определяет отображать ли стрелку
    property VisibleScale: boolean read FVisibleScale write SetVisibleScale; // определяет отображать ли шкалу
    property IndentMin: integer read FIndentMin write SetIndentMin; // опреляет отступ оси от края компонента, пикселов
    property IndentMax: integer read FIndentMax write SetIndentMax; // опреляет отступ оси от противоположного края компонента, пикселов
    property ScaleStep: variant read FScaleStep write SetScaleStep; // шаг шкалы
    property ScalePointReference: variant read FScalePointReference
      write SetScalePointReference; // нулевая точка шкалы (точка привязки)
    property AxisType: TAxisType read FAxisType write SetAxisType;  // тип координатной оси (абцисс или ординат)
    ...
    property Distance: integer read FDistance write SetDistance;    // отступ надписей шкалы от оси, пикселов
    ...
    property ScaleSize: integer read FScaleSize write SetScaleSize; // длина черточек шкалы, пикселов
    property ScaleEmptyIntervalMin: variant
      read FScaleEmptyIntervalMin write SetScaleEmptyIntervalMin;   // задание границ интервала на координатной оси..
    property ScaleEmptyIntervalMax: variant
      read FScaleEmptyIntervalMax write SetScaleEmptyIntervalMax;   //..на котором шкала не отображается
  ...
  private
    ...
    function GetMinX: variant; override;
    procedure SetMinX(Value: variant); override;
    function GetMinY: variant; override;
    procedure SetMinY(Value: variant); override;
    function GetMaxX: variant; override;
    procedure SetMaxX(Value: variant); override;
    function GetMaxY: variant; override;
    procedure SetMaxY(Value: variant); override;
...
type
  ...
  private
    ...
    FLegend: TLegend;
    function GetColorBackground: TColor; override;
...
constructor TCustomDiagram.Create(AOwner: TComponent);
begin
...
procedure TCustomDiagram.Paint;
var
  Save: Boolean;
begin
...
function TCustomDiagram.DestRect: TRect;
begin
  if Stretch then
    Result := ClientRect
  else if Center then
...
procedure TCustomDiagram.SetStretch(Value: Boolean);
begin
...
procedure TCustomDiagram.SetCenter(Value: Boolean);
begin
...
procedure TCustomDiagram.SetPicture(Value: TPicture);
begin
  FPicture.Assign(Value);
end;

function TCustomDiagram.GetCanvas: TCanvas;
var
  Bitmap: TBitmap;
begin
...
begin
  ...
  if csDesigning in AOwner.ComponentState then
    begin
     ...
     if (AOwner as TDiagram).Parent<>nil
       then
         (AOwner as TDiagram).Paint;
   end;
end;
...
procedure TCurve.SetMinX(Value: variant);
begin
  FMinX := Value;
end;

procedure TCurve.SetMinY(Value: variant);
begin
  FMinY := Value;
end;

procedure TCurve.SetMaxX(Value: variant);
begin
  FMaxX := Value;
end;

procedure TCurve.SetMaxY(Value: variant);
begin
  FMaxY := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      try
        X1 := (Width * (Point[PointCount - 1].X - FMinX)) / (FMaxX - FMinX);
        Y1 := Height - (Height * (Point[PointCount - 1].Y - FMinY)) / (FMaxY - FMinY);
        X2 := (Width * (X - FMinX)) / (FMaxX - FMinX);
        Y2 := Height - (Height * (Y - FMinY)) / (FMaxY - FMinY);
        ix1 := Round(X1);
        iy1 := Round(Y1);
        ix2 := Round(X2);
        iy2 := Round(Y2);
...
procedure TCurve.SetVisible(Value: boolean);
begin
  FVisible := Value;
end;
...
procedure TGrid.SetGridStepX(Value: variant);
begin
  FGridStepX := Value;
end;

procedure TGrid.SetGridStepY(Value: variant);
begin
  FGridStepY := Value;
end;
...
procedure TAxis.SetCoordinate(Value: variant);
begin
  FCoordinate := Value;
end;

procedure TAxis.SetArrowWidth(Value: integer);
begin
  FArrowWidth := Value;
end;

procedure TAxis.SetArrowLeng(Value: integer);
begin
  FArrowLeng := Value;
end;

procedure TAxis.SetVisible(Value: boolean);
begin
  FVisible := Value;
end;

procedure TAxis.SetVisibleArrow(Value: boolean);
begin
  FVisibleArrow := Value;
end;

procedure TAxis.SetIndentMin(Value: integer);
begin
  FIndentMin := Value;
end;

procedure TAxis.SetIndentMax(Value: integer);
begin
  FIndentMax := Value;
end;

procedure TAxis.SetScaleStep(Value: variant);
begin
  FScaleStep := Value;
end;

procedure TAxis.SetScalePointReference(Value: variant);
begin
  FScalePointReference := Value;
end;
...
procedure TAxis.SetDistance(Value: integer);
begin
  FDistance := Value;
end;

procedure TAxis.SetVisibleScale(Value: boolean);
begin
  FVisibleScale := Value;
end;

procedure TAxis.SetScaleSize(Value: integer);
begin
  FScaleSize := Value;
end;

procedure TAxis.SetScaleEmptyIntervalMin(Value: variant);
begin
  FScaleEmptyIntervalMin := Value;
end;

procedure TAxis.SetScaleEmptyIntervalMax(Value: variant);
begin
  FScaleEmptyIntervalMax := Value;
end;
...
begin
  ...
  begin
    FWidth := Self.Width;
    FHeight := Self.Height;
    Picture.Bitmap.Width:=FWidth;
    Picture.Bitmap.Height:=FHeight;
    ReDraw;
  end;
  inherited Paint;
end;
...
function TCurve.GetMaxX: variant;
begin
  Result := FMaxX;
end;

function TCurve.GetMaxY: variant;
begin
  Result := FMaxY;
end;

function TCurve.GetMinX: variant;
begin
  Result := FMinX;
end;

function TCurve.GetMinY: variant;
begin
  Result := FMinY;
end;

function TCurve.GetVisible: boolean;
begin
  Result := FVisible;
end;
...
function TDiagram.GetColorBackground: TColor;
begin
...
begin
  ...
  begin
    Curve.Diagram.Canvas.Pen := Pen;
    yc := round(Curve.Diagram.Height - (Curve.Diagram.Height *
      (Coordinate - Curve.MinY)) / (Curve.MaxY - Curve.MinY));
    ...
    begin
      ...
      Curve.Diagram.Canvas.Polygon([Point(Curve.Diagram.Width - IndentMax, yc),
        Point(Curve.Diagram.Width - IndentMax - ArrowLeng, yc -
        (ArrowWidth shr 1) - correction), Point(Curve.Diagram.Width -
        IndentMax - ArrowLeng, yc + (ArrowWidth shr 1)),
        Point(Curve.Diagram.Width - IndentMax, yc - correction)]);
    end;{if}
        ...
        begin
          ...
          if (x < ScaleEmptyIntervalMin) or (x > ScaleEmptyIntervalMax) then
            ...
            begin
              ...
              Curve.Diagram.Canvas.TextOut(xc -
                (Curve.Diagram.Canvas.TextWidth(st) shr 1), yc +
                Distance + i * (Curve.Diagram.Canvas.TextHeight(st)), st);
            end;{for}
          FStringList.Free;
        end;{if}
        x := x + ScaleStep;
...
begin
  ...
  begin
    ...
    begin
      ...
      if VisibleScale then
      begin
        xc := (Curve.Diagram.Width * (Coordinate - Curve.MinX)) /
          (Curve.MaxX - Curve.MinX);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Delete(st, 1, 1);
              FStringList.Free;
              Curve.Diagram.Canvas.TextOut(xc - Distance -
                Curve.Diagram.Canvas.TextWidth(st), yc -
                (Curve.Diagram.Canvas.TextHeight(st) shr 1), st);
            end;
          end;{if}
          y := y + ScaleStep;
        end;{while}
...
begin
  ...
  begin
    ...
    if FLegend.Style = lsWindow then       // отрисовываем рамку
      Canvas.Rectangle(FLeft, FLegend.Top, FLeft + FWidth, FLegend.Top + FHeight);
    // и нарисуем рамку
    ix := 0;
    ...
    begin
      ...
      begin
        ...
        Canvas.Rectangle(FLeft + 2, FLegend.Top + ix * FTextHeight +
          2, FLeft + FTextHeight,
          FLegend.Top + ix * FTextHeight + FTextHeight);
        Canvas.Brush.Color := FColorBackground;
        Canvas.TextOut(FLeft + FTextHeight + 2, FLegend.Top + ix *
          FTextHeight + 1, ' - ' + CurveIx[i].ID);
        Inc(ix);
      end;
    end;
  end;
...
begin
  Width := Diagram.Width;
  Height := Diagram.Height;
  MX := round((Width * (X - FMinX)) / (FMaxX - FMinX));
  MY := round((Height - (Height * (Y - FMinY))) / (FMaxY - FMinY));
  Diagram.Canvas.Pen := Pen;
      ...
      if MarkerCount>0 then
       for ix := 0 to MarkerCount - 1 do
        begin
          ...
          writeln(SVGFile,  'style="fill:#'+ColorToStrSVG(FMarkerColor)+';fill-rule:evenodd;stroke:#000000;stroke-width:1px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1" />');
        end; {for}
      end;
      Dec(i);
    end;
    ...
    for i := 0 to CurveCount - 1 do // находим кривую с самым длинным названием
      if CurveIx[i].Visible then
        if Canvas.TextWidth(st) < Canvas.TextWidth(CurveIx[i].ID) then
          st := CurveIx[i].ID;
    st := ' - ' + st;
    FTextWidth := round(Canvas.TextWidth(st)*1.2); // и устанавливаем ширину строки
    FTextHeight := abs(FLegend.Font.Height); // и высоту строки
    FWidth := FTextWidth + FTextHeight + 8;
    ix := 0;
    for i := 0 to CurveCount - 1 do
      if CurveIx[i].Visible then
        Inc(ix); // посчитаем сколько у нас видимых кривых
    ...
    begin
      ...
      begin
        ...
        SaveText(FLeft + FTextHeight + 4, FLegend.Top + (ix + 1) * FTextHeight -
          2, 'text11' + IntToStr(i), ' - ' + AnsiToUtf8(CurveIx[i].ID), FLegend.Font);
        Inc(ix);
      end;
    end;
    writeln(SVGFile, '</g>');
